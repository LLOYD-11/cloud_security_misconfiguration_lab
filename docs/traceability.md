...
| M9-R6 | Add a report preview, demo walkthrough, and learning reflection. | Complete | The [report preview](assets/report-preview.svg), [five-minute demo](demo-walkthrough.md), and [documented lessons](contribution-and-reflection.md#lessons-learned) present real project evidence without widening the documented claims. |
...
| M12-R3 | Compare overlapping behavior with an established external baseline. | Complete | Complete [external-baseline audit](evaluation-baselines.md), frozen [35-rule overlap matrix](../evaluation/baseline-overlap-v1.0.json), 24 label-independent exact-overlap [baseline decisions](../evaluation/baseline-outcomes-v1.0.json), five hashed Prowler source predicates, the hashed 12-rule Sigma Core AWS release inventory, supplemental outcome schema, candidate-independent replay verifier, adversarial tests, and CI/release/package gates. |
| M12-R4 | Publish classification metrics, disagreement analysis, and relevant ablations. | Complete | Immutable [machine-readable results](../evaluation/results-v1.0.json), deterministic [evaluation runner](../tools/evaluation_runner.py), release-safe [isolated replay](../tools/evaluation_replay.py), replay and tamper tests, CI/release/package gates, and the complete [evaluation report](evaluation-report.md). The frozen candidate recorded 77 TP, 2 FP, 1 FN, 90 TN, 0.9809 F1, exact severity for 77/77 matched positives, 24/24 exact-overlap Prowler agreement, and 8/8 native/simplified agreement. Acceptance remained false because storage precision was 0.8824 and two predictions were unlabelled; the zero-case network-context ablation is disclosed as non-estimable. |
...
| M13-R4 | Produce a reviewer-first README, short real-system demo, and one-page summary. | In progress | Reviewer-first [README](../README.md), focused [CLI reference](cli-reference.md), and [one-page application summary](application-summary.md) are complete. The [real-system protocol](real-system-demo.md), minimal [CloudFormation template](../demo/real-system/network-demo-stack.json), and safety regressions are prepared, but no live AWS result is claimed before authorized execution, sanitization, and teardown evidence. |
