...
| External baseline outcomes | [`evaluation-baseline-outcomes-v1.0.schema.json`](../schemas/evaluation-baseline-outcomes-v1.0.schema.json) |
| Independent evaluation results | [`evaluation-results-v1.0.schema.json`](../schemas/evaluation-results-v1.0.schema.json) |

The schemas use JSON Schema Draft 2020-12. Contract tests validate every committed sample, the built-in rule catalog, the benchmark manifest, the frozen evaluation protocol, corpus manifest, and primary result, and analyzer-generated findings, incident, analysis-summary, remediation-plan, attack-timeline, and benchmark-result files against these schemas.
...
The [frozen evaluation corpus](evaluation-corpus.md) adds executable semantic,
provenance, hash, exact-inventory, pair-completeness, and output-blind review
checks beyond what JSON Schema alone can express. The
[baseline audit](evaluation-baselines.md) adds a complete rule-level overlap
matrix, source-audited outcome replay, explicit exclusions, and deterministic
tamper verification. The [primary result](evaluation-report.md) adds
candidate replay, raw-decision reconciliation, canonical-byte verification,
and complete failed-acceptance disclosure.
...
Simplified file inputs cross a dependency-free runtime validator before
analysis. It verifies every structure and primitive the analyzers consume,
including nested policy, credential, bucket, rule, reachability, identity, and
event fields; it also checks RFC 3339 timestamps, CIDRs, port ordering, and
unambiguous documented compatibility aliases. Accepted timestamp offsets are
canonicalized to UTC `Z`. Errors identify the first invalid value with a stable
JSON-style path. Full Draft 2020-12 evaluation remains an
